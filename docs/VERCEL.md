...
| `NEXT_PUBLIC_SITE_URL` | **Consigliato in produzione** | Dominio ufficiale: `https://gmcsandonatese.com` (senza slash finale). Usato per Open Graph, canonical, sitemap e JSON-LD. Se omesso su Vercel si usa `VERCEL_URL` (preview); in build/produzione senza Vercel il fallback nel codice è `https://gmcsandonatese.com`. |
